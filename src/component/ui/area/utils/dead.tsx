...
export const Dead: React.FC<Props> = ({ size }: Props) => {
    ...
    return (
        <Circle
            ...
            scale={{
                y: dead ? ((getSizeInPixel(size) / 2) / ((dead?.height > dead.width ? dead.width : dead.height) / 2)) * .8 : 0,
                x: dead ? ((getSizeInPixel(size) / 2) / ((dead?.height > dead.width ? dead.width : dead.height) / 2)) * .8 : 0,
            }}
        />
    )
}
