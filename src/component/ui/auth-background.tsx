...
export const AuthBackground: React.FC<Props> = ({ text }: Props) => {
    ...
    return (
        <div className={cn("h-full w-1/2 z-20 top-0 absolute transition-07", (!boolean && 'translate-x-full'))} onClick={() => !text && swap()}>
            <div className="h-full items-center cursor-pointer flex justify-center arrow-auth-relative">
                <div className="rounded-md h-full w-full bg-shadow bg-img" style={{ backgroundImage: `url(${'/img/auth.jpg'})` }} ></div>
